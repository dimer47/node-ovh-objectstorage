...
class Objects {
  ...
  download (path, pathLocal) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const writeStream = fs.createWriteStream(pathLocal);
  ...
  async delete_with_result (path, checkContainer = true) {
    try {
      await this.context.objects().delete(path, checkContainer);
      return true;
    } catch (e) {
      return false;
    }
  ...
  deletes (paths, checkContainer = true) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const deletes = await Promise.all(_.map(paths, (path) => {
  ...
  expire_at (path, expire_date) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (_.isUndefined(expire_date)) {
          // noinspection ExceptionCaughtLocallyJS
          throw new Error("Datetime expiration is expected.");
        }
        if (!_.isDate(expire_date) && !moment(expire_date).isValid()) {
          // noinspection ExceptionCaughtLocallyJS
  ...
  expire_after (path, delete_seconds) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (_.isUndefined(delete_seconds)) {
          // noinspection ExceptionCaughtLocallyJS
          throw new Error("Delete seconds isn't defined.");
        }
        if (!_.isNumber(delete_seconds)) {
          // noinspection ExceptionCaughtLocallyJS
