        ...
        if (!_.isUndefined(types)) {
          switch (types) {
            ...
            case "static":
              ...
              headers["x-container-meta-web-error"] = (!_.isUndefined(web_content_pages.error) ? web_content_pages.error : "error.html");
              headers["x-container-meta-web-listings-css"] = (!_.isUndefined(web_content_pages.css) ? web_content_pages.css : "listing.css");
              headers["x-container-meta-web-index"] = (!_.isUndefined(web_content_pages.index) ? web_content_pages.index : "index.html");
              break;
          }
        }
  ...
  delete_objects (container) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        // get file list
        const files = await this.context.containers().list(container);

        // deletes files
        const deletes = await this.context.objects().deletes(_.map(files, (file) => {
