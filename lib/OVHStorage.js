    ...
    return {
      ...
      get(path, pathLocal = null, callback, failback) {
        try {
          ...
          /*var r = request({
            ...
            if(pathLocal !== null){
              //console.log(body.toString('binary'));
              console.log(new Buffer(body.toString("utf8")));
              console.log(md5(body));
              fs.writeFile(pathLocal, body,function(err) {
                if (err){ return callback(false); }
                return callback(fs.existsSync(pathLocal));
              });
            }
      ...
      set(file, path, callback, failback) {
        try {
          ...
          console.log(md5(fs.readFileSync(file, "utf8")));
          stream.pipe(request({
            method:   'PUT',
            uri:      targetURL,
            headers:  headers
